...
class OBB_PosePredictor(DetectionPredictor):
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        for i, pred in enumerate(preds):
            ...
            str_preds = str(pred)
            with open("/home/new_drive_2/ultralytics-main/ultralytics/engine/prediction_postprocess_preds.txt", "w") as file:
                        file.write(str_preds)
            results.append(Results(orig_img, path=img_path, names=self.model.names, obb=obb, keypoints=pred_kpts))

            indices = torch.ones(pred_kpts.size(0), dtype=torch.bool)


            for k, pred_kpt in enumerate (pred_kpts):

                if pred_kpts[k][0][0].item() == 0.0 and pred_kpts[k][0][1].item() == 0.0:
                    indices[k] = False

            pred_kpts=pred_kpts[indices]
            obb=obb[indices]
